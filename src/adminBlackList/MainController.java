...
public class MainController implements Initializable {
   ...
   */
    ...
    void sel_bt_view() {
		...
		tableViewColumn2.setCellValueFactory(new PropertyValueFactory("strId"));//table의 첫번째 컬럼을 strid와대응시킨다.
    	...
    	for (selectTotalDTO total : totals) { // 테이블들어갈 내용을 만드는 부분
			 ...
			 * }
			 */
			strRecord[0] = total.getMemberId();
			strRecord[1] = Integer.toString(total.getMemberTime());
			strRecord[2] = Integer.toString(total.getLockerTime());
			strRecord[3] = "";
			strRecord[4] = total.getBlackList();					
    ...
    public void regBlackList() {
    	apiLoginDAO loginDao = new apiLoginDAO();
    	String dbId = loginDao.loginProc(hpNumber.getText());
    	if(dbId == null) {
    		CommonService.msg("등록을 하세요");
    		
    	}else {
    		System.out.println("f");
    		selectTotalDAO dao = new selectTotalDAO();
    		dao.blackListInsert(dbId);
    ...
    public void delBlackList() {
    	...
    	String dbId = loginDao.loginProc(hpNumber.getText());
    	if(dbId == null) {
    		CommonService.msg("휴대폰 번호가 없습니다.");
    		
    	}else {
    		System.out.println("f");
    		selectTotalDAO dao = new selectTotalDAO();
    		dao.blackListDelete(dbId);
